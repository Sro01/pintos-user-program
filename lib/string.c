...
/* SRC에서 DST로 SIZE 바이트를 복사합니다. 겹치면 안 됩니다.
   DST를 반환합니다. */
void *
...
/* SRC에서 DST로 SIZE 바이트를 복사합니다. 겹쳐도 됩니다.
   DST를 반환합니다. */
void *
...
/* A와 B의 SIZE 바이트 블록에서 첫 번째 다른 바이트를 찾습니다.
...
/* BLOCK에서 시작하는 첫 SIZE 바이트에서 CH의 첫 번째 발생 위치에 대한 포인터를 반환합니다.
...
/* STRING에서 C의 첫 번째 발생 위치를 찾고 반환합니다.
   C가 STRING에 나타나지 않으면 null 포인터를 반환합니다.
   C == '\0'이면 STRING의 끝에 있는 null 종결자에 대한 포인터를 반환합니다. */
char *
...
/* STRING의 초기 하위 문자열 길이를 반환합니다.
   이 하위 문자열은 STOP에 없는 문자로 구성됩니다. */
size_t
...
strtok_r (char *s, const char *delimiters, char **save_ptr) {
	...
	while (strchr (delimiters, *s) != NULL) {
		/* strchr()는 null 바이트를 검색할 때 항상 null이 아닌 값을 반환합니다.
		   모든 문자열은 null 바이트를 포함하기 때문입니다 (끝에). */
		if (*s == '\0') {
			*save_ptr = s;
			return NULL;
		}
...
/* 문자열 SRC를 DST로 복사합니다. SRC가 SIZE - 1 문자보다 길면
   SIZE - 1 문자만 복사됩니다. null 종결자는 항상 DST에 기록됩니다.
   SIZE가 0인 경우는 제외합니다. null 종결자를 포함하지 않은 SRC의 길이를 반환합니다.

   strlcpy()는 표준 C 라이브러리에 포함되어 있지 않지만 점점 인기를 얻고 있는 확장입니다.
...
/* 문자열 SRC를 DST에 연결합니다. 연결된 문자열은 SIZE - 1 문자로 제한됩니다.
   null 종결자는 항상 DST에 기록됩니다. SIZE가 0인 경우는 제외합니다.
   null 종결자를 포함하지 않은 충분한 공간이 있다고 가정할 때 연결된 문자열의 길이를 반환합니다.

   strlcat()는 표준 C 라이브러리에 포함되어 있지 않지만 점점 인기를 얻고 있는 확장입니다.
   strlcpy()에 대한 정보는 http://www.courtesan.com/todd/papers/strlcpy.html을 참조하세요. */
size_t
