...
/* initd 및 다른 프로세스를 위한 일반적인 프로세스 초기화기. */
static void
process_init (void) {
	printf("3️⃣ process_init\n");
...
process_create_initd (const char *file_name) {
	 ...
	 * 그렇지 않으면 호출자와 load() 사이에 경쟁 상태가 발생합니다. */
	...
	printf("1️⃣ process_create_initd\n");
...
#ifndef VM
/* 부모의 주소 공간을 복제하여 pml4_for_each에 이 함수를 전달합니다.
 * 이는 프로젝트 2에만 해당됩니다. */
static bool
duplicate_pte (uint64_t *pte, void *va, void *aux) {
	...
	/* 3. TODO: 자식에게 새 PAL_USER 페이지를 할당하고 결과를
	 *    TODO: NEWPAGE에 설정합니다. */

	/* 4. TODO: 부모의 페이지를 새 페이지로 복제하고
	 *    TODO: 부모의 페이지가 쓰기 가능한지 여부를 확인합니다
	 *    TODO: (결과에 따라 WRITABLE을 설정합니다). */

	/* 5. 자식의 페이지 테이블에 주소 VA로 새 페이지를 WRITABLE
	 *    권한으로 추가합니다. */
...
__do_fork (void *aux) {
	 ...
	 * TODO: 부모는 이 함수가 부모의 리소스를 성공적으로 복제할 때까지
...
process_exec (void *f_name) {
	...
	for (token = strtok_r(file_name, " ", &save_ptr); token != NULL; token = strtok_r(NULL, " ", &save_ptr))
		parse[count++] = token; // 파일 이름을 공백으로 구분하여 토큰으로 저장


	/* 그런 다음 바이너리를 로드합니다. */
	success = load (file_name, &_if);
	// 이진 파일을 디스크에서 메모리로 로드
	// 로드된 후 실행할 메인 함수의 시작 주소 필드 초기화 (if_.rip)
	// user stack의 top 포인터 초기화 (if_.rsp)
	// 위 과정을 성공하면 실행을 계속하고, 실패하면 스레드가 종료됨
...
void argument_stack(char **parse, int count, void **rsp) // 주소를 전달받았으므로 이중 포인터 사용
{
	...
	for (int i = count - 1; i > -1; i--)
	{
		...
		parse[i] = *(char **)rsp; // parse[i]에 현재 rsp의 값을 저장해둠(지금 저장한 인자가 시작하는 주소값)
	}
 ...
 *
 * 이 함수는 문제 2-2에서 구현될 것입니다. 현재로서는 아무것도 하지 않습니다. */
int
...
process_exit (void) {
	 ...
	 * TODO: 프로세스 리소스 정리를 여기에 구현하는 것을 권장합니다. */
...
process_cleanup (void) {
	...
	if (pml4 != NULL) {
		 ...
		 * cur->pagedir를 NULL로 설정해야 페이지 디렉토리를 전환하기 전에
		 * 타이머 인터럽트가 프로세스 페이지 디렉토리로 전환할 수 없습니다.
		 * 프로세스의 페이지 디렉토리를 파괴하기 전에 기본 페이지 디렉토리를 활성화해야 합니다.
		 * 그렇지 않으면 활성 페이지 디렉토리가 해제되고(지워진) 페이지 디렉토리가 됩니다. */
		curr->pml4 = NULL;
		pml4_activate (NULL);
		pml4_destroy (pml4);
	}
...
validate_segment (const struct Phdr *phdr, struct file *file) {
	...
	/* 영역이 커널 가상 주소 공간을 가로질러 "랩 어라운드"할 수 없습니다. */
	if (phdr->p_vaddr + phdr->p_memsz < phdr->p_vaddr)
		return false;

	/* 페이지 0을 매핑하는 것을 금지합니다.
	   페이지 0을 매핑하는 것은 나쁜 생각일 뿐만 아니라
	   허용하면 null 포인터를 시스템 호출에 전달한 사용자 코드가
	   memcpy() 등의 null 포인터 단언을 통해 커널을 패닉시킬 수 있습니다. */
...
#ifndef VM
/* 이 블록의 코드는 프로젝트 2 동안에만 사용됩니다.
 * 프로젝트 2 전체에 대해 함수를 구현하려면 #ifndef 매크로 외부에 구현하세요. */

/* load() 도우미. */
static bool install_page (void *upage, void *kpage, bool writable);

/* FILE의 OFS에서 시작하여 주소 UPAGE에 세그먼트를 로드합니다.
 * 총 READ_BYTES + ZERO_BYTES 바이트의 가상 메모리가 초기화됩니다.
 *
 * - UPAGE에서 시작하여 FILE에서 READ_BYTES 바이트를 읽어야 합니다.
 *
 ...
 * WRITABLE이 true인 경우 사용자 프로세스는 페이지를 수정할 수 있으며,
 * 그렇지 않은 경우 읽기 전용입니다.
 * UPAGE는 이미 매핑되어 있지 않아야 합니다.
 * KPAGE는 아마도 사용자 풀에서 palloc_get_page()로 얻은 페이지여야 합니다.
 * 성공 시 true를 반환하고, UPAGE가 이미 매핑되어 있거나 메모리 할당에 실패하면 false를 반환합니다. */
static bool
 ...
 * 프로젝트 2에 대해서만 함수를 구현하려면 위 블록에 구현하세요. */
...
/* USER_STACK에 PAGE의 스택을 생성합니다. 성공 시 true를 반환합니다. */
static bool
setup_stack (struct intr_frame *if_) {
	bool success = false;
